...
subjectfolder_in='C:\Users\yanghj\Box\Yang Lab\Projects\Cardiac Delayed Phase DCE\Data\Structured data\Animals\Lisbon\Lisbon_wk8+2';
...
%fitpointsset={[1:3], [2:4], [2:11], [2:length(timein)],[2 4 6]};
fitpointsset={[2:4]};
...
DCE_Scatterplot(fitresultsDCEetk(:,:,1:3),RemoteMask,MIMask,MVOMask)
...
F_cxm=fitresultsDCEcxm(:,:,3);
B=fitresultsDCEcxm(:,:,4);
c=m2-B.*(m2-m1);
b=m1.*m2./c;
vp_cxm=c./F_cxm;
a=m1+m2-b;
PS_cxm=a.*vp_cxm-F_cxm;
ve_cxm=PS_cxm./b;
...
figure;imagesc(F_cxm);axis equal;title(['F Con#=',num2str(length(fitpointind))]);caxis([0 1])
figure;imagesc(vp_cxm);axis equal;title(['vp Con#=',num2str(length(fitpointind))]);caxis([0 10])
figure;imagesc(PS_cxm);axis equal;title(['PS Con#=',num2str(length(fitpointind))]);caxis([0 10])
%figure;imagesc(fitresultsDCE(:,:,1)./fitresultsDCE(:,:,2).*HeartMask);axis equal;title(['Ve Con#=',num2str(length(fitpointind))])
figure;imagesc(ve_cxm);axis equal;title(['ve Con#=',num2str(length(fitpointind))]);caxis([0 10])
figure;imagesc(Rsq_cxm);axis equal;title(['Rsq Con#=',num2str(length(fitpointind))])
