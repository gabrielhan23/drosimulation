...
function []=Run_Process_cardiacDCE_unit_func(subjectfolder_in)  
...
RemoteMask=roipoly;
    ...
    save([subjectfolder,'\DCE\ROI.mat'],'Blood', 'HeartMask', 'LVendo', 'Myomask', 'MIMask', 'MVOMask', 'RemoteMask')
...
fitpointsset={ [1 2 3], [1 3 4],[1:2:length(timein)],[1:length(timein)]};

%fitpointsset={[1 2 3], [2 3 4], [1 3 4], [2:length(timein)], [1:length(timein)],[1 2 4 8],[1:2:length(timein)]};
...
while(NityFivpercent<0.9 & iter<8) % initial guess optimization
...
for c=1:sum(HeartMask(:))
   ...
   tempfitDCE= fitdcemri_etk(sigin',Cp',time',x0ctemp(c,:),lb,ub,'etk_b');
   %tempfitDCE= fitdcemri_etk(sigin',Cp',time',x0,lb,ub,'etk');
...
%
tekb_Ktrans=fitresultsDCEetk(:,:,1);%0-1
tekb_Ve=fitresultsDCEetk(:,:,2); %0-0.5 
tekb_Vp=fitresultsDCEetk(:,:,3)*100;% 0-100 percentage
tekb_Kep=fitresultsDCEetk(:,:,1)./fitresultsDCEetk(:,:,2).*HeartMask; % 0-100 percentage
tekb_Rsq=fitresultsDCEetk(:,:,end);
%%
figure;
subplot(3,2,1);imagesc(tekb_Ktrans);axis equal;title(['tek_Ktrans Con#=T',num2str(timepoints),'min']);caxis([0 1]);xlim([1 size(fitresultsDCEetk,1)]);ylim([1 size(fitresultsDCEetk,2)]);
subplot(3,2,2);imagesc(tekb_Kep);axis equal;title(['tek_Kep Con#=T',num2str(timepoints),'min']);caxis([0 1]);xlim([1 size(fitresultsDCEetk,1)]);ylim([1 size(fitresultsDCEetk,2)]);
subplot(3,2,3);imagesc(tekb_Ve);axis equal;title(['tek_Ve Con#=T',num2str(timepoints),'min']);caxis([0 1]);xlim([1 size(fitresultsDCEetk,1)]);ylim([1 size(fitresultsDCEetk,2)]);
subplot(3,2,4);imagesc(tekb_Vp);axis equal;title(['tek_Vp Con#=T',num2str(timepoints),'min']);caxis([0 50]);xlim([1 size(fitresultsDCEetk,1)]);ylim([1 size(fitresultsDCEetk,2)]);
subplot(3,2,5);imagesc(tekb_Rsq);axis equal;title(['tek_Rsq Con#=T',num2str(timepoints),'min']);caxis([0 1]);xlim([1 size(fitresultsDCEetk,1)]);ylim([1 size(fitresultsDCEetk,2)]);
%}
...
while(NityFivpercent<0.85 &iter<3) % initial guess optimization

    
    %Etkve
    etkve=tek_Ve(HeartMask);
    etkvp=tek_Vp(HeartMask);
    etkrsq=tek_Rsq(HeartMask);
for c=1:sum(HeartMask(:))
   %use ETK Ve to bound CXM model 
    if etkrsq(c)>0.8
    x0ve=etkve(c);
    x0ve=min(max(x0ve, 0),100);
    x0vp=etkvp(c);
    x0vp=min(max(x0vp, 0),100);
    x0=[0.5 0.1 x0vp x0ve];
    x0c=repmat(x0,[sum(HeartMask(:)) 1]);
    Rsqc=-1*ones([sum(HeartMask(:)) 1]);
    lb=[0 -1 min(max((0.5*x0vp), 0),100) min(max((x0ve-0.05*x0ve), 0),100)];
    ub=[5 1 min(max((x0vp+0.5*x0vp), 0),100) min(max((x0ve+0.05*x0ve), 0),100)];
   end
   %results=a*exp(-bt)+d*exp(-et) and gof
   if Rsqc(c)<0.9   
   % x0=x0c(c,:).*((1-(rand(1,4)-0.5)*1.5));
...
subplot(3,2,4);imagesc(vp_cxm);axis equal;title(['vp_cxm Con#=T',num2str(timepoints),'min']);caxis([0 20]);xlim([1 size(fitresultsDCEcxm,1)]);ylim([1 size(fitresultsDCEcxm,2)]);

subplot(3,2,5);imagesc(Rsq_cxm);axis equal;title(['Rsq_cxm Con#=T',num2str(timepoints),'min']);caxis([0.7 1]);xlim([1 size(fitresultsDCEcxm,1)]);ylim([1 size(fitresultsDCEcxm,2)]);
%% mixed fitting 2CXM
...
subplot(3,2,4);imagesc(vp_cxmmix);axis equal;title(['vp_cxmmix Con#=T',num2str(timepoints),'min']);caxis([0 20]);xlim([1 size(fitresultsDCEcxm,1)]);ylim([1 size(fitresultsDCEcxm,2)]);

subplot(3,2,5);imagesc(Rsq_cxmmix);axis equal;title(['Rsq_cxm Con#=T',num2str(timepoints),'min']);caxis([0.7 1]);xlim([1 size(fitresultsDCEcxm,1)]);ylim([1 size(fitresultsDCEcxm,2)]);
%%
%{
figure;title(['T',num2str(timepoints),'min'])
